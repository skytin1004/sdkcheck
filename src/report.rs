...
pub fn write_json_report(report: &AuditReport, output: &Path, secrets: &SecretSet) -> Result<()> {
...
pub fn render_markdown_report(report: &AuditReport) -> String {
    ...
    push_list(&mut markdown, "Audit Steps", &report.audit_steps);
    push_list(
        &mut markdown,
        "Docs Observations",
        &report.docs_observations,
    );
    push_list(&mut markdown, "Provided Env Names", &report.provided_envs);
    push_list(&mut markdown, "Missing Env Names", &report.missing_envs);
...
mod tests {
    ...
    use crate::models::{AuditReport, FailureClassification, ReportStatus};
    ...
    fn renders_secret_names_without_values() {
        let report = AuditReport {
            ...
            provided_envs: vec!["OPENAI_API_KEY".to_string()],
            missing_envs: vec![],
            commands: vec![],
            generated_files: vec![],
            suggestions: vec![],
