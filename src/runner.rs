...
pub struct CommandRunner {
    ...
    docker_venv_volume: Option<String>,
    timeout: Duration,
}
...
impl CommandRunner {
    ...
    pub fn with_docker_network(mut self, network: Option<String>) -> Self {
        self.docker_network = network;
        self
    }

    pub fn with_docker_venv_volume(mut self, volume: Option<String>) -> Self {
        self.docker_venv_volume = volume;
        self
    }
    ...
    fn run_docker(&self, spec: CommandSpec) -> Result<CommandResult> {
        ...
        if let Some(volume) = &self.docker_venv_volume {
            command
                .arg("--mount")
                .arg(format!("type=volume,source={volume},target=/venv"));
        }

        command.arg("--network");
        if let Some(network) = &self.docker_network {
            command.arg(network);
        } else {
            command.arg("bridge");
        }
